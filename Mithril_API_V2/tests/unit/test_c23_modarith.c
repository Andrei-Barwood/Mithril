...
static void switch_to_c23(mithril_context *ctx) {
    mithril_status st = mithril_provider_activate(ctx, "c23");
    expect_ok(st, "activate c23 provider");
}

static void test_add_mod_small(mithril_context *ctx) {
    const uint8_t a[] = {0x5D};    /* 93 */
    const uint8_t b[] = {0x22};    /* 34 */
    const uint8_t m[] = {0x65};    /* 101 */
    const uint8_t expected[] = {0x1A}; /* 26 */
    uint8_t out[8] = {0};
    size_t written = 0u;
...
static void test_mul_mod_small(mithril_context *ctx) {
    const uint8_t a[] = {0x12}; /* 18 */
    const uint8_t b[] = {0x0E}; /* 14 */
    const uint8_t m[] = {0x17}; /* 23 */
    const uint8_t expected[] = {0x16}; /* 22 */
    uint8_t out[8] = {0};
    size_t written = 0u;
...
static void test_mul_mod_large(mithril_context *ctx) {
    const uint8_t a[] = {0x01,0x00,0x00,0x00,0x01}; /* 2^32 + 1 */
    const uint8_t b[] = {0x00,0x00,0x00,0x02};      /* 2 */
    const uint8_t m[] = {0x01,0x00,0x00,0x00,0x07}; /* 2^32 + 7 */
    const uint8_t expected[] = {0xFF,0xFF,0xFF,0xFB}; /* (2*(2^32+1)) mod (2^32+7) = 2^32-5 */
    uint8_t out[16] = {0};
    size_t written = 0u;
...
int main(void) {
    ...
    if (st == MITHRIL_ERR_NOT_IMPLEMENTED) {
        puts("[SKIP] test_c23_modarith (sodium provider not enabled at build time)");
        return 0;
    }
